...
def get_combined_data():
    ...
    tickers = ['ETH-USD', 'SHY']
    df = yf.download(tickers, start="2017-11-09", progress=False)
    ...
    try:
        spread = web.DataReader('T10Y2Y', 'fred', start='2017-11-09')
    except:
        print("⚠️ FRED 연결 실패. yfinance 데이터로 대체 시도.")
        t10 = yf.download("^TNX", start="2017-11-09", progress=False)['Close']
        t02 = yf.download("^IRX", start="2017-11-09", progress=False)['Close']
        spread = t10 - t02

    # 코인 시장은 365일 열리지만, 금리와 SHY는 평일만 존재하므로 ffill로 채워줍니다.
    df = df.join(spread).fillna(method='ffill')
    df.columns = list(df.columns[:-1]) + ['T10Y2Y']
    df = df.dropna()

    # 수익률 정의
    df['ETH_Pct'] = df['ETH-USD'].pct_change()      # 본주 (1배)
    df['Sim_Lev_2X'] = df['ETH_Pct'] * 2.0         # ETHU 가상 (2배)
    df['Sim_Cash'] = df['SHY'].pct_change()         # 현금 (SHY)

    # RSI 14 계산
    delta = df['ETH-USD'].diff()
...
def run_strategy(df, ma_period, rsi_limit, sell_buffer):
    ma = df['ETH-USD'].ewm(span=ma_period, adjust=False).mean()
    
    price_arr = df['ETH-USD'].values
    ma_arr = ma.values
    ...
    for i in range(1, len(df)):
        ...
        if price < sell_threshold:
            ...
            if price > ma_val:
                ...
                # RSI 스위칭: 과열 시 본주로 전환하여 하락 변동성 대비
                if rsi_val < rsi_limit:
                    is_leveraged[i] = 1 # 2배 모드
                else:
                    is_leveraged[i] = 0 # 1배 본주 모드
            
            # 매크로 필터: 금리 역전 시 비중 제한 및 1배 고정
            if spread_val < 0:
    ...
    ret_spot = df['ETH_Pct'] * pos * (1 - is_lev)
...
def analyze_today(df, ma_period, rsi_limit, sell_buffer, total_score):
    last_row = df.iloc[-1]
    price = last_row['ETH-USD']
    ma_series = df['ETH-USD'].ewm(span=ma_period, adjust=False).mean()
    ...
    print("\n" + "═"*60)
    print(f"🏆 [최적화 완료: ETH ↔ ETHU 스위칭 전략]")
    ...
    print("-" * 60)
    mode_msg = "🚀 ETHU(2배) 모드" if target_lev == 1 else "🛡️ ETH(본주) 스위칭 모드"
...
if __name__ == "__main__":
    ...
    if df_raw is not None:
        ...
        print(f"\n⚡ {total_comb:,}개 조합 분석 중... 이더리움은 데이터량이 많아 시간이 소요될 수 있습니다.")
