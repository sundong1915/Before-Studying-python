...
# ==========================================
# 1. 데이터 수집 (UNH, SHY, 장단기 금리차)
# ==========================================
def get_combined_data():
    print("⏳ 데이터 수집 중... (UNH, SHY, 10Y-2Y Spread)")
    
    tickers = ['UNH', 'SHY']
    # 2006년부터 현재까지의 데이터를 수집합니다.
    ...
    # 수익률 정의
    df['UNH_Pct'] = df['UNH'].pct_change()      # 본주 (1배)
    df['Sim_Lev_2X'] = df['UNH_Pct'] * 2.0     # 가상 레버리지 (2배)
...
def run_strategy(df, ma_period, rsi_limit, sell_buffer):
    ma = df['UNH'].ewm(span=ma_period, adjust=False).mean()
    
    price_arr = df['UNH'].values
    ma_arr = ma.values
    ...
    for i in range(1, len(df)):
        ...
        if price < sell_threshold:
            ...
            if price > ma_val:
                ...
                # RSI 스위칭: 기준 미달 시 2배, 기준 초과(과열) 시 1배 본주
    ...
    ret_spot = df['UNH_Pct'] * pos * (1 - is_lev)
...
def analyze_today(df, ma_period, rsi_limit, sell_buffer, total_score):
    last_row = df.iloc[-1]
    price = last_row['UNH']
    ma_series = df['UNH'].ewm(span=ma_period, adjust=False).mean()
    ...
    print("\n" + "═"*60)
    print(f"🏆 [최적화 완료: UNH ↔ UNHU 스위칭 전략]")
    ...
    print(f"🎯 [현재 유나이티드헬스 지표]")
    print(f"   • UNH 현재가 : ${price:,.2f}")
    ...
    print("-" * 60)
    mode_msg = "🚀 2배 레버리지 모드" if target_lev == 1 else "🛡️ 1배 본주 스위칭 모드"
...
if __name__ == "__main__":
    ...
    if df_raw is not None:
        ...
        print(f"\n⚡ {total_comb:,}개 조합 정밀 분석 시작... UNH의 20년 데이터를 탐색합니다.")
