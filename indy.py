...
# ==========================================
# 1. 데이터 수집 (INDY, SHY, 장단기 금리차)
# ==========================================
def get_combined_data():
    print("⏳ 데이터 수집 중... (INDY, SHY, 10Y-2Y Spread)")
    
    tickers = ['INDY', 'SHY']
    # INDY(iShares India 50 ETF) 상장 시점(2009년 말) 데이터를 최대한 확보하기 위해 2008년부터 탐색합니다.
    ...
    try:
        spread = web.DataReader('T10Y2Y', 'fred', start='2008-01-01')
    except:
        print("⚠️ FRED 연결 실패. yfinance 데이터로 대체 시도.")
        t10 = yf.download("^TNX", start="2008-01-01", progress=False)['Close']
        t02 = yf.download("^IRX", start="2008-01-01", progress=False)['Close']
        spread = t10 - t02
    ...
    # 수익률 정의
    df['INDY_Pct'] = df['INDY'].pct_change()      # 본주 (1배)
    df['Sim_Lev_2X'] = df['INDY_Pct'] * 2.0      # 가상 2배 레버리지 (ex. INDL)
...
def run_strategy(df, ma_period, rsi_limit, sell_buffer):
    ma = df['INDY'].ewm(span=ma_period, adjust=False).mean()
    
    price_arr = df['INDY'].values
    ma_arr = ma.values
        ...
        if price < sell_threshold:
            ...
            if price > ma_val:
                ...
                if rsi_val < rsi_limit:
                    is_leveraged[i] = 1 # 2배 모드 (ex. INDL)
                else:
                    is_leveraged[i] = 0 # 1배 모드 (INDY)
    ...
    ret_spot = df['INDY_Pct'] * pos * (1 - is_lev)
...
def analyze_today(df, ma_period, rsi_limit, sell_buffer, total_score):
    last_row = df.iloc[-1]
    price = last_row['INDY']
    ma_series = df['INDY'].ewm(span=ma_period, adjust=False).mean()
    ...
    print("\n" + "═"*60)
    print(f"🏆 [최적화 완료: INDY ↔ 가상 2배(INDL) 스위칭 전략]")
    ...
    print(f"🎯 [현재 인도 증시(INDY) 지표]")
    print(f"   • INDY 현재가 : ${price:,.2f}")
    ...
    print("-" * 60)
    mode_msg = "🚀 2배 레버리지 모드" if target_lev == 1 else "🛡️ INDY(본주) 스위칭 모드"
...
if __name__ == "__main__":
    ...
    if df_raw is not None:
        ...
        print(f"\n⚡ {total_comb:,}개 조합 정밀 분석 중... 인도 증시(INDY)의 과거 데이터를 탐색합니다.")
