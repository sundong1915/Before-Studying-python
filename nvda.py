...
# ==========================================
# 1. 데이터 수집 (NVDA & 장단기 금리차)
# ==========================================
def get_combined_data():
    print("⏳ 데이터 수집 중... (NVDA, SHY, 10Y-2Y Spread)")
    ...
    try:
        # FRED에서 장단기 금리차 직접 로드
        spread = web.DataReader('T10Y2Y', 'fred', start='2006-01-01')
    except:
        print("⚠️ FRED 연결 실패. yfinance 국채 데이터로 대체 시도.")
    ...
    # 수익률 정의
    df['NVDA_Pct'] = df['NVDA'].pct_change()      # 본주 (1배)
    df['Sim_Lev_2X'] = df['NVDA_Pct'] * 2.0     # NVDL 가상 (2배)
...
def run_strategy(df, ma_period, rsi_limit, sell_buffer):
    ma = df['NVDA'].ewm(span=ma_period, adjust=False).mean()
    
    price_arr = df['NVDA'].values
    ma_arr = ma.values
    ...
    for i in range(1, len(df)):
        ...
        if price < sell_threshold:
            ...
            if price > ma_val:
                ...
                # RSI 스위칭: 기준 미달 시 2배(NVDL), 기준 초과 시 1배(NVDA)
                if rsi_val < rsi_limit:
                    is_leveraged[i] = 1 
                else:
                    is_leveraged[i] = 0 
            
            # 금리차 역전 시: 비중 30% 제한 + 무조건 1배(NVDA)
            if spread_val < 0:
    ...
    ret_spot = df['NVDA_Pct'] * pos * (1 - is_lev)
...
def analyze_today(df, ma_period, rsi_limit, sell_buffer, total_score):
    last_row = df.iloc[-1]
    price = last_row['NVDA']
    ma_series = df['NVDA'].ewm(span=ma_period, adjust=False).mean()
    ...
    print("\n" + "═"*60)
    print(f"🏆 [최적화 완료: NVDA ↔ NVDL 스위칭 전략]")
    ...
    print(f"   • 최적 RSI 기준 : {rsi_limit} (이 수치 넘으면 본주 스위칭)")
    ...
    print(f"🎯 [현재 시장 지표 및 범위]")
    print(f"   • NVDA 현재가 : ${price:.2f}")
    print(f"   • 현재 EMA({ma_period}) 가격 : ${current_ma:.2f}")
    print(f"   • 매도 감시선 : ${sell_line:.2f} (남은 여유: {dist_to_sell:.2f}%)")
    print(f"   • 현재 RSI 수치 : {rsi:.1f}")
    print(f"   • 현재 장단기 금리차 : {current_spread:.4f} ({spread_status})")
    
    print("-" * 60)
    mode_msg = "🚀 NVDL(2배) 모드" if target_lev == 1 else "🛡️ NVDA(본주) 스위칭 모드"
    if target_pos == 0: mode_msg = "🚨 전량 현금(SHY) 대피"
