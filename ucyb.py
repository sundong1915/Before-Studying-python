...
# ==========================================
# 1. 데이터 수집 (CIBR, SHY, 장단기 금리차)
# ==========================================
def get_combined_data():
    print("⏳ 데이터 수집 중... (CIBR, SHY, 10Y-2Y Spread)")
    
    tickers = ['CIBR', 'SHY']
    # 사이버보안 섹터(CIBR)의 상장일(2015년 7월)을 고려해 2015년부터 수집합니다.
    ...
    try:
        spread = web.DataReader('T10Y2Y', 'fred', start='2015-01-01')
    except:
        print("⚠️ FRED 연결 실패. yfinance 데이터로 대체 시도.")
        t10 = yf.download("^TNX", start="2015-01-01", progress=False)['Close']
        t02 = yf.download("^IRX", start="2015-01-01", progress=False)['Close']
        spread = t10 - t02
    ...
    # 수익률 정의
    df['CIBR_Pct'] = df['CIBR'].pct_change()      # 본주 (1배)
    df['Sim_Lev_2X'] = df['CIBR_Pct'] * 2.0       # UCYB 가상 (2배 레버리지)
...
def run_strategy(df, ma_period, rsi_limit, sell_buffer):
    ma = df['CIBR'].ewm(span=ma_period, adjust=False).mean()
    
    price_arr = df['CIBR'].values
    ma_arr = ma.values
        ...
        if price < sell_threshold:
            ...
            if price > ma_val:
                ...
                if rsi_val < rsi_limit:
                    is_leveraged[i] = 1 # UCYB(2배) 모드
                else:
                    is_leveraged[i] = 0 # CIBR(본주) 모드
    ...
    ret_spot = df['CIBR_Pct'] * pos * (1 - is_lev)
...
def analyze_today(df, ma_period, rsi_limit, sell_buffer, total_score):
    last_row = df.iloc[-1]
    price = last_row['CIBR']
    ma_series = df['CIBR'].ewm(span=ma_period, adjust=False).mean()
    ...
    print("\n" + "═"*60)
    print(f"🏆 [최적화 완료: CIBR ↔ UCYB 스위칭 전략]")
    ...
    print(f"🎯 [현재 사이버보안 ETF(CIBR) 지표]")
    print(f"   • CIBR 현재가 : ${price:,.2f}")
    ...
    print("-" * 60)
    mode_msg = "🚀 UCYB(2배) 모드" if target_lev == 1 else "🛡️ CIBR(본주) 스위칭 모드"
...
if __name__ == "__main__":
    ...
    if df_raw is not None:
        ...
        print(f"\n⚡ {total_comb:,}개 조합 정밀 분석 중... 사이버보안 섹터의 역사를 탐색합니다.")
