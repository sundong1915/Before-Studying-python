...
# ==========================================
# 1. 데이터 수집 (LLY, SHY, 장단기 금리차)
# ==========================================
def get_combined_data():
    print("⏳ 데이터 수집 중... (LLY, SHY, 10Y-2Y Spread)")
    
    tickers = ['LLY', 'SHY']
    # 일라이릴리의 메가트렌드(비만치료제 등) 성장을 반영하기 위해 2006년부터 수집합니다.
    ...
    # 수익률 정의
    df['LLY_Pct'] = df['LLY'].pct_change()       # 본주 (1배)
    df['Sim_Lev_2X'] = df['LLY_Pct'] * 2.0       # LLY 가상 레버리지 (2배)
...
def run_strategy(df, ma_period, rsi_limit, sell_buffer):
    ma = df['LLY'].ewm(span=ma_period, adjust=False).mean()
    
    price_arr = df['LLY'].values
    ma_arr = ma.values
        ...
        if price < sell_threshold:
            ...
            if price > ma_val:
                ...
                if rsi_val < rsi_limit:
                    is_leveraged[i] = 1 # LLY 2배 모드
                else:
                    is_leveraged[i] = 0 # LLY 본주 모드
    ...
    ret_spot = df['LLY_Pct'] * pos * (1 - is_lev)
...
def analyze_today(df, ma_period, rsi_limit, sell_buffer, total_score):
    last_row = df.iloc[-1]
    price = last_row['LLY']
    ma_series = df['LLY'].ewm(span=ma_period, adjust=False).mean()
    ...
    print("\n" + "═"*60)
    print(f"🏆 [최적화 완료: LLY ↔ LLY(2X) 스위칭 전략]")
    ...
    print("-" * 60)
    mode_msg = "🚀 LLY 2X(레버리지) 모드" if target_lev == 1 else "🛡️ LLY(본주) 스위칭 모드"
...
if __name__ == "__main__":
    ...
    if df_raw is not None:
        ...
        print(f"\n⚡ {total_comb:,}개 조합 정밀 분석 중... 일라이릴리의 20년 역사를 탐색합니다.")
